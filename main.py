...
class Win_Main(QMainWindow):
    ...
    def show_confusion_matrix(self):
        y_true, y_pred = decode_one_hot(SI.y_true, SI.model.result)
    ...
    def handle_save_results(self):
        # 当前没有模型或者预测结果为空
        if not SI.model or not len(SI.model.result):
        ...
        if file_name:
            ...
            self.thread_save_result = Thread_Save_Result(file_name, SI.model.result)
    ...
    def action_load_model(self):
        ...
        if model_name:
            # 如果点击了确认按钮，则启动线程加载模型
            self.thread = Thread_Load_Model(model_name)
            self.thread.start()
        else:
            signal.progress_bar.emit(0)
    ...
    def handle_save_model(self):
        ...
        if file_name:
            # 启动线程保存预测模型
            self.thread_save_model = Thread_Save_Model(file_name, SI.model.result)
